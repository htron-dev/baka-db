# Hunter x Hunter: Greed Island Final

-   **type**: ova
-   **episodes**: 14
-   **original-name**: ハンター × ハンター　 G・I 　 Final
-   **start-date**: 2004-03-03
-   **end-date**: 2004-03-03
-   **opening-song**: "Believe in Tomorrow" by Sunflower's Garden
-   **ending-song**: "Moshimo Kono Sekai de Kimi to Boku ga Deaenakatta Nara" by Sunflower's Garden
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   action
-   adventure
-   super-power
-   fantasy
-   shounen

## Sinopse

Continuing with their adventure at Greed Island, Gon and Killua train furiously under the guidance of Biscuit for better preparation at attempting the game. With their abilities vastly improving and the threat of player killers menacing Greed Island, Gon and company continue the game despite the dangers that they will face with the game activities and encounters with other players. Based on the manga by Togashi Yoshihiro.

## Links

-   [My Anime list](https://myanimelist.net/anime/139/Hunter_x_Hunter__Greed_Island_Final)
-   [Official Site](http://www.hunter-ova.com/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=1427)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=3177)
-   [Wikipedia](http://en.wikipedia.org/wiki/Special:Search/Hunter_x_Hunter)
