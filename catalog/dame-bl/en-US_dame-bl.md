# Dame BL

-   **type**: manga
-   **volumes**: 1
-   **chapters**: 12
-   **original-name**: ダメ BL
-   **start-date**: 2011-11-25

## Tags

-   yaoi

## Authors

-   est em (Story & Art)
-   Takeuchi
-   Sachiko (Story & Art)
-   Chiba
-   Ryouko (Story & Art)
-   Cotorino
-   Deathco (Story & Art)
-   Momoyama
-   Naoko (Story & Art)
-   Okadaya
-   Tetuzoh (Story & Art)
-   Psyche
-   Delico (Story & Art)
-   Morozumi
-   Sumitomo (Story & Art)
-   Kumota
-   Haruko (Story & Art)
-   ZIN (Story & Art)
-   Aoyama
-   Tohmi (Story & Art)
-   Mochimeko (Story & Art)
-   Kevin
-   Komine (Story & Art)
-   Umematsu
-   Machie (Story & Art)
-   Rotta
-   Ikumi (Story & Art)

## Sinopse

Dame BL is "a collection of BL oneshots written by some of the lesser-known participants in the genre (and some you may be familiar with already), with a focus on less conventional settings and characters. We're very far away from high school here, which hopefully you'll agree makes a pleasant change of pace."

1. Be Here to Love Me (Kumota Haruko)
2. Kou no Kawa (Morozumi Sumitomo)
3. Khaa Thoong (est em)
4. Natsu no Saigo ni Yoset (Umematsu Machie)
5. Locas! (Okadaya Tetuzoh)
6. Andorobo (Chiba Ryouko)
7. Shinumade BL (Takeuchi Sachiko)
8. Tonari wa Nani wo Suru Hito zo (Uno Jinia)
9. Soldier (Zin)
10. Muhou no Shounen (Mochimeko)
11. Ima Doushiteru (Rotta Ikumi)
12. It Must Be (Momoyama Naoko)

(Source: acme)

Note: Story 1 was collected in Bara no Mori ni Ita Koro

## Links

-   [My Anime list](https://myanimelist.net/manga/39557/Dame_BL)
