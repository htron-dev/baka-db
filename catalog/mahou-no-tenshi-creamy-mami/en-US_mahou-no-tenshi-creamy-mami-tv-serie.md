# Mahou no Tenshi Creamy Mami

-   **type**: tv-serie
-   **episodes**: 52
-   **original-name**: 魔法の天使 クリィミーマミ
-   **start-date**: 1983-07-01
-   **end-date**: 1983-07-01
-   **opening-song**: "Delicate ni Sukishite (デリケートに好きして)" by Takako Oota
-   **ending-song**: #1: "Pajama no Mama de (パジャマのままで)" by Takako Oota (eps 1-26, 52)
-   **rating**: G - All Ages

## Tags

-   comedy
-   fantasy
-   magic
-   romance
-   school
-   sci-fi
-   shoujo

## Sinopse

Creamy Mami is about a young girl, Yuu, who after seeing a spaceship is given the power to use magic for one year. She is also given 2 cats, Poji and Nega, to watch over and guide her. Using her magic powers to transform into the idol Creamy Mami, Yuu must work hard at acting, singing, helping her parents at their crepe shop, fighting aliens and bad guys, going to school, plus try to get the affections of her childhood friend Toshio.

(Source: ANN)

## Links

-   [My Anime list](https://myanimelist.net/anime/2044/Mahou_no_Tenshi_Creamy_Mami)
-   [Official Site](http://pierrot.jp/title/magicgirl/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=521)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=1284)
-   [Wikipedia](http://en.wikipedia.org/wiki/Creamy_Mami,_the_Magic_Angel)
