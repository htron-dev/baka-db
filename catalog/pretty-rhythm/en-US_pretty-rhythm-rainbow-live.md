# Pretty Rhythm: Rainbow Live

-   **type**: tv-serie
-   **episodes**: 51
-   **original-name**: プリティーリズム・レインボーライブ
-   **start-date**: 2013-04-06
-   **end-date**: 2013-04-06
-   **opening-song**: #1: "BOY MEETS GIRL" by Prizmmy☆ (eps 1-13)
-   **ending-song**: #1: "RainBowxRainBow" by Prism☆Box (eps 1-13)
-   **rating**: PG - Children

## Tags

-   slice-of-life
-   sports
-   music
-   shoujo

## Sinopse

Naru Ayase is an 8th grader who can see the colors of music when she listens to it. For Naru, who is extremely good at decorating, becoming the owner of a shop like Dear Crown was her dream. One day, she finds out that the manager of a newly-opened shop is recruiting middle school girls who can do Prism Dance, and immediately applies. Naru begins to Prism Dance at the audition, and an aura she's never experienced spreads out in front of her. At that moment, a mysterious girl named Rinne asks her if she can see "rainbow music."

(Source: ANN)

## Links

-   [My Anime list](https://myanimelist.net/anime/17249/Pretty_Rhythm__Rainbow_Live)
-   [Official Site](http://www.tv-tokyo.co.jp/anime/prettyrhythm/rl/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=9664)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=15094)
