# Isekai Quartet

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: 異世界かるてっと
-   **start-date**: 2019-04-10
-   **end-date**: 2019-04-10
-   **opening-song**: "Isekai Quartet (異世界かるてっと)" by Ainz (Satoshi Hino), Kazuma (Jun Fukushima), Subaru (Yuusuke Kobayashi), Tanya (Aoi Yuuki)
-   **ending-song**: #1: "Isekai Girls♡Talk (異世界ガールズ ♡ トーク)" by Albedo (Yumi Hara), Aqua (Sora Amamiya), Emilia (Rie Takahashi), Tanya (Aoi Yuuki) (eps 1-4, 6-10)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   comedy
-   fantasy
-   parody

## Sinopse

It is a normal day; everyone from deranged military girl Tanya Degurechaff and 16-year-old isekai protagonist Satou Kazuma to expansionist overlord Ainz Ooal Gown and demon sisters Rem and Ram go on with their daily lives. Suddenly, a conspicuous red button begging to be pressed appears before them. Overcome by curiosity, the otherworldly characters push the button, sending them to an unfamiliar world.

With no way of escaping, the characters must lead a normal school life and make acquaintances with the others. One thing is certain: the classroom full of fan-favorite eccentric personalities never gets boring!

[Source My Anime List]

## Links

-   [My Anime list](https://myanimelist.net/anime/38472/Isekai_Quartet)
-   [Official Site](http://isekai-quartet.com/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=14435)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=21992)
-   [Wikipedia](https://en.wikipedia.org/wiki/Isekai_Quartet)
