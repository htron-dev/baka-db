# Last Period: Owarinaki Rasen no Monogatari

![last-period-owarinaki-rasen-no-monogatari](https://cdn.myanimelist.net/images/anime/1710/95065.jpg)

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: ラストピリオド -終わりなき螺旋の物語-
-   **start-date**: 2018-04-12
-   **end-date**: 2018-04-12
-   **opening-song**: "Yokubari Dreamer (欲張り Dreamer)" by Haru (Natsuki Hanae) x Choco (Yukari Tamura)
-   **ending-song**: "Wiseman no Theme (ワイズマンのテーマ)" by Wiseman (Mizaru (Sayaka Harada), Kikazaru (Akari Kito), and Iwazaru (Ayumi Mano))
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   action
-   adventure
-   comedy
-   fantasy
-   magic

## Sinopse

"Period" is how magic users called who beat "Spiral"—monsters that were summoned from isolation. Due to the rise of these beings, 14-year-old apprentice Period Haru, who is a part of the Eighth Arc-end Division, is called to break the cycle and cast himself into the endless battle. However, a mysterious thievery occurred and sank the division into bankruptcy, forcing Haru and his other comrades have to leave their headquarters. To rebuild a branch, they have to overcome quest after quest.

(Source: MAL News)

## Links

-   [My Anime list](https://myanimelist.net/anime/37254/Last_Period__Owarinaki_Rasen_no_Monogatari)
-   [Official Site](http://www.lastperiod.jp/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=13779)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=20612)
-   [Wikipedia](http://en.wikipedia.org/wiki/Last_Period)
