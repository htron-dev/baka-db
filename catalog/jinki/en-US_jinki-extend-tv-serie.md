# Jinki:Extend

![jinki-extend](https://cdn.myanimelist.net/images/anime/3/44792.jpg)

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: ジンキ・エクステンド
-   **start-date**: 2005-01-06
-   **end-date**: 2005-01-06
-   **opening-song**: "FLY AWAY" by unicorn table
-   **ending-song**: "Mirai to Yuu na no Kotae" by angela
-   **rating**: R - 17+ (violence & profanity)

## Tags

-   mecha
-   sci-fi

## Sinopse

Aoba is a young girl who loves to build models of robots. She lived alone with her grandmother until her grandmother passes away. Shortly after she is kidnapped and brought to a secret base where she discovers a huge robot. The piloted robots fight against Ancient-Jinki in The Grand Savanna, but the true meaning behind the fights is hidden. Aoba works hard at the base so one day she can pilot one of the robots and discover these secrets.

## Links

-   [My Anime list](https://myanimelist.net/anime/141/Jinki_Extend)
-   [Official Site](http://www.jinki.info/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=2742)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=4658)
-   [Wikipedia](http://en.wikipedia.org/wiki/Jinki:Extend)
