# Try Knights

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: トライナイツ
-   **start-date**: 2019-07-31
-   **end-date**: 2019-07-31
-   **opening-song**: "Mugen no Try (無限のトライ)" by Shougo Sakamoto
-   **ending-song**: "Moyou (模様)" by Ivy to Fraudulent Game
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   sports
-   school
-   josei

## Sinopse

Riku Haruma enters high school without a future in sight. He sees people playing like monsters on the ground, jumping higher than anyone else, running fast, deciding to try their best. There, he sees Akira Kariya playing rugby, a sport he was once passionate about but gave up due to his physique. While Akira is running roughly, Riku gives him a piece of advice without thinking. From that, Riku feels his dying passion for the sport set ablaze, and his future starts to brighten up...

(Source: MAL News)

## Links

-   [My Anime list](https://myanimelist.net/anime/39527/Try_Knights)
-   [Official Site](https://www.ntv.co.jp/tryknights/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=14751)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=21956)
