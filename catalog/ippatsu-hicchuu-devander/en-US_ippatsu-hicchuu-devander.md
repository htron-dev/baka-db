# Ippatsu Hicchuu!! Devander

![ippatsu-hicchuu-devander](https://cdn.myanimelist.net/images/anime/4/89823.jpg)

-   **type**: ova
-   **episodes**: 1
-   **original-name**: 一発必中!! デバンダー
-   **start-date**: 2012-10-19
-   **opening-song**: "Kimi no Devander (君のデバンダー)" by Kenichi Takemoto (竹本健一)
-   **ending-song**: "Niji no Sekai e (虹の世界へ) by Rio (凛生)
-   **rating**: G - All Ages

## Tags

-   action
-   sci-fi
-   parody
-   super-power
-   mecha

## Sinopse

This story revolves around a spirited, carefree boy named Kazuma Harukaze, who lives with his little sister Sakura and his horse Lovely on a farm within sight of Mt. Fuji. One day, a "meteor" lands nearby, and it turns out to be Hinkeeru—a messenger from Muzanda who has come to warn Earth of a grave danger—and a robot horse named Robony.

(Source: ANN)

## Links

-   [My Anime list](https://myanimelist.net/anime/13693/Ippatsu_Hicchuu_Devander)
-   [Official Site](http://www.tatsunoko.co.jp/devander/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=9082)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=14171)
-   [Wikipedia](http://ja.wikipedia.org/wiki/一発必中!!_デバンダー)
