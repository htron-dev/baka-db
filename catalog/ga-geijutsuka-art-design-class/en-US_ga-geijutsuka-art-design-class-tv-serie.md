# GA: Geijutsuka Art Design Class

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: GA 芸術科アートデザインクラス
-   **start-date**: 2009-07-07
-   **end-date**: 2009-07-07
-   **opening-song**: "Osaki ni s'il vous plaît (お先にシルブプレ)" by Saiwai Koukou GA girls
-   **ending-song**: #1: "Coloring palettes Tomokane-iro (トモカネいろ)" by Sawashiro Miyuki (eps 1, 6)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   slice-of-life
-   comedy
-   school
-   seinen

## Sinopse

The Ayanoi High School features the Geijutsuka Art Design Class (GA) that focuses on the arts. Five close friends — the energetic "hime"-prankster Noda Miki; the level-headed, cynical Nozaki Namiko; the intelligent, observant, and kind Oomichi Miyabi; the lively and mischievous tomboy Tomokane; and the curious, innocent, glasses-wearing Yamaguchi Kisaragi — attend this class with great enthusiasm, learning about the many art techniques. Every day seems to pose a new and interesting challenge, be it struggling with the latest assignment or when dealing with the daily strangeness of school life.

(Source: AniDB)

## Links

-   [My Anime list](https://myanimelist.net/anime/5670/GA__Geijutsuka_Art_Design_Class)
-   [Official Site](http://mv.avex.jp/ga/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=6574)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=10895)
-   [Wikipedia](http://en.wikipedia.org/wiki/GA_Geijutsuka_Art_Design_Class)
