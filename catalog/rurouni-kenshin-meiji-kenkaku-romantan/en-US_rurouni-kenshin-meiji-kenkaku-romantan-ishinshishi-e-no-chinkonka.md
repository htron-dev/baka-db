# Rurouni Kenshin: Meiji Kenkaku Romantan - Ishinshishi e no Chinkonka

![rurouni-kenshin-meiji-kenkaku-romantan-ishinshishi-e-no-chinkonka](https://cdn.myanimelist.net/images/anime/4/47845.jpg)

-   **type**: movie
-   **episodes**: 1
-   **original-name**: るろうに剣心 -明治剣客浪漫譚- 維新志士への鎮魂歌
-   **start-date**: 1997-12-20
-   **opening-song**: "Niji (虹)" by L'arc-en-Ciel
-   **ending-song**: "Towa no Mirai (永遠の未来 - Eternal Future)" by Animetal
-   **rating**: R - 17+ (violence & profanity)

## Tags

-   samurai
-   historical
-   drama
-   shounen

## Sinopse

The war against the Tokugawa Shogunate ended years ago. But there are some who are not happy with the outcome. Shigure Takimi watched his friends and family get slashed down in the name of freedom and prosperity. Now he and a band of desperate rebels have sworn to settle one final score. Only one man stands in their way: Kenshin Himura. But when Shigure discovers Kenshin's true identity as the Hitokiri Battousai, his fight becomes a personal vendetta.

(Source: ANN)

## Links

-   [My Anime list](https://myanimelist.net/anime/46/Rurouni_Kenshin__Meiji_Kenkaku_Romantan_-_Ishinshishi_e_no_Chinkonka)
-   [Official Site](http://en.wikipedia.org/wiki/Rurouni_Kenshin:_Ishin_Shishi_he_no_Requiem)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=422)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=217)
-   [Wikipedia](http://en.wikipedia.org/wiki/Rurouni_Kenshin#Requiem_for_the_Ishin_Patriots)
