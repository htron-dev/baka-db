# Tsubasa: Tokyo Revelations

![tsubasa-tokyo-revelations](https://cdn.myanimelist.net/images/anime/4/49129.jpg)

-   **type**: ova
-   **episodes**: 3
-   **original-name**: ツバサ TOKYO REVELATIONS
-   **start-date**: 2007-11-16
-   **end-date**: 2007-11-16
-   **opening-song**: "Synchronicity" by Makino Yui
-   **ending-song**: "Saigo no Kajitsu" by Sakamoto Maaya
-   **rating**: R - 17+ (violence & profanity)

## Tags

-   action
-   adventure
-   drama
-   romance
-   fantasy
-   shounen

## Sinopse

Continuing their journey from Record Country (the place that held the book of memories), Syaoran and company land into Tokyo Country, a blood-filled country under war from opposing factions for natural resources and survival. While reluctantly involved in the race to live, the group faces their own problems as certain revelations are made, changing their journey forever.

A shocking betrayal and a battle that risks all of their lives. After this revelation, they will never be the same again.

## Links

-   [My Anime list](https://myanimelist.net/anime/2685/Tsubasa__Tokyo_Revelations)
-   [Official Site](http://www.shonenmagazine.com/tsubasa_tokyo/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=5301)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=8292)
-   [Wikipedia](http://en.wikipedia.org/wiki/Tsubasa:_Reservoir_Chronicle)
