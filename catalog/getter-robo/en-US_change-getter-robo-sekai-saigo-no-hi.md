# Change!! Getter Robo: Sekai Saigo no Hi

![change-getter-robo-sekai-saigo-no-hi](https://cdn.myanimelist.net/images/anime/4/23767.jpg)

-   **type**: ova
-   **episodes**: 13
-   **original-name**: 真(チェンジ!!)ゲッターロボ 世界最後の日
-   **start-date**: 1998-08-25
-   **end-date**: 1998-08-25
-   **opening-song**: #1: "Ima ga Sono Toki da (This Is the Time)" by Ichiro Mizuki (eps 1-3)
-   **ending-song**: #1: "The Hill of Promise" by Satomi Nakase and Chiemi Chiba (eps 1-3)
-   **rating**: R - 17+ (violence & profanity)

## Tags

-   action
-   psychological
-   adventure
-   mecha
-   sci-fi
-   horror
-   shounen

## Sinopse

Dr. Saotome returns from the dead after being seemingly murdered by Getter pilot Nagare Ryoma and threatens the world with his ultimate creation: the Shin Dragon. Ryoma reluctantly teams up with his former co-pilots Jin Hayato and Tomoe Mushashi to combat Dr. Saotome. The pilots must also deal with the mysterious Invaders, aliens that have the ability to possess humans and warp their flesh to form hideous creatures.

(Source: Nihon Review)

## Links

-   [My Anime list](https://myanimelist.net/anime/978/Change_Getter_Robo__Sekai_Saigo_no_Hi)
-   [Official Site](http://www.bandaivisual.co.jp/change_getter/shin_getter.html)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=949)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=20)
-   [Wikipedia](http://en.wikipedia.org/wiki/Getter_Robo_Armageddon)
