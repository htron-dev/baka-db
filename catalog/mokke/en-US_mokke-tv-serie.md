# Mokke

![mokke](https://cdn.myanimelist.net/images/anime/13/19674.jpg)

-   **type**: tv-serie
-   **episodes**: 24
-   **original-name**: もっけ
-   **start-date**: 2007-10-03
-   **end-date**: 2007-10-03
-   **opening-song**: "Kokoro no Ato" by Riyu Kosaka
-   **ending-song**: "Panorama (パノラマ)" by Mai Hashimoto
-   **rating**: PG - Children

## Tags

-   slice-of-life
-   supernatural
-   mystery

## Sinopse

This is a story about two sisters: Shizuru, is a high school student who is able to see ghosts while her younger sister, Mizuki, is haunted by these apparitions. Frustrated by their abilities, their parents decided to entrust the sisters into the care of their grandparents who live in the countryside. As they adapt to life in the countryside, Shizuru and Mizuki begin to learn about the importance of coexisting nature with these apparitions.

(Source: ANN)

## Links

-   [My Anime list](https://myanimelist.net/anime/2931/Mokke)
-   [Official Site](http://www.avexmovie.jp/lineup/mokke/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=5052)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=7611)
-   [Wikipedia](http://en.wikipedia.org/wiki/Mokke)
