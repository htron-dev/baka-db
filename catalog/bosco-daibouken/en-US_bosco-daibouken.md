# Bosco Daibouken

-   **type**: tv-serie
-   **episodes**: 26
-   **original-name**: ボスコアドベンチャー
-   **start-date**: 1986-10-06
-   **end-date**: 1986-10-06
-   **opening-song**: "Tokimeki wa Forever (ときめきは Forever)" by Noriko Hidaka
-   **ending-song**: "Hareta Hi ni mo Ai wo Kudasai (晴れた日にも愛をください)" by Noriko Hidaka
-   **rating**: PG - Children

## Tags

-   adventure
-   fantasy

## Sinopse

Based on the book by Tony Wolf.

The Fountain of Life is the source of all life forces in the Bosco World. Princess Apricot, who is supposed to become Queen of Fountainland-the Guardian of the Fountain of Life-is kidnapped by villains who aim to rule the Bosco World.

If Princess Apricot can't take the queen's throne by the next solar eclipse, the Fountain of Life will dry up. The animals living in the forest of Bosco, including Frog, Otter and Tortoise, decide to help Princess and take her to Fountainland in order to protect their peaceful life in the Bosco World. So begins the journey in a balloon full of dreams, hope, adventures, and love!

Source: Official Website)

## Links

-   [My Anime list](https://myanimelist.net/anime/3460/Bosco_Daibouken)
-   [Official Site](http://www.nippon-animation.co.jp/work/bosco_adventure.html)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=2107)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=1229)
-   [Wikipedia](http://en.wikipedia.org/wiki/Bosco_Adventure)
