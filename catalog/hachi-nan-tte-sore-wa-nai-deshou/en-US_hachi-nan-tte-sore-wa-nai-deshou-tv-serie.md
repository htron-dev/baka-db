# Hachi-nan tte, Sore wa Nai deshou!

![hachi-nan-tte-sore-wa-nai-deshou](https://cdn.myanimelist.net/images/anime/1834/110718.jpg)

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: 八男って、それはないでしょう！
-   **start-date**: 2020-04-02
-   **end-date**: 2020-04-02
-   **opening-song**: "Jikuu no Mayoi Hito (時空の迷い人)" by Demon Kakka and Arika Takarano (ALI PROJECT)
-   **ending-song**: "Tsukiagari no Monologue (月明かりの Monologue)" by AKINO arai and AKINO from bless4
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   action
-   fantasy

## Sinopse

Waking up in a new world, 25-year-old Shingo Ichinomiya realizes that he is in the body of a six-year-old. Retaining memories of his stressful life working at a firm company, Shingo learns that the person he is occupying is Wendelin Von Benno Baumeister, the eighth son of a poor noble family living in the countryside. Awoken to his bizarre situation, Wendelin strives to change his financial and social status for the better. His newly discovered great magical aptitude may prove to be just what he needs to achieve that goal.

[Source My Anime List]

## Links

-   [My Anime list](https://myanimelist.net/anime/38830/Hachi-nan_tte_Sore_wa_Nai_deshou)
-   [Official Site](http://hachinan-anime.com/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=14535)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=22285)
-   [Wikipedia](https://en.wikipedia.org/wiki/The_8th_Son%3F_Are_You_Kidding_Me%3F#Anime)
