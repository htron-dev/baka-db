# Inu to Hasami wa Tsukaiyou

![inu-to-hasami-wa-tsukaiyou](https://cdn.myanimelist.net/images/anime/13/56313.jpg)

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: 犬とハサミは使いよう
-   **start-date**: 2013-07-02
-   **end-date**: 2013-07-02
-   **opening-song**: "Wan Wan Wan Wan N_1!! (わんわんわんわん N_1!!)" by Inu Musume Club (犬っ娘クラブ)
-   **ending-song**: "Lemonade Scandal (レモネイドスキャンダル)" by Maxi Akizuki (CV: Yuu Serizawa)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   comedy
-   supernatural

## Sinopse

A nonsense comical mystery. Harumi Kazuhito is a high school boy who loves books and is a fan of novelist Natsuno Kirihime. One day, he finds Kirihime writing at a cafe, about to be shot by a robber. He protects her from the attack but is killed instead. Through the supernatural power of a book-worm, he is reincarnated as a dachshund dog. Kazuhito (as a dog) writhes in a painful bookless life, when a sadistic woman carrying a pair of scissors offers him help. She is Kirihime herself.

(Source: Dog and Scissors Wiki)

## Links

-   [My Anime list](https://myanimelist.net/anime/17831/Inu_to_Hasami_wa_Tsukaiyou)
-   [Official Site](http://inuhasa.jp/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=9663)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=15101)
-   [Wikipedia](http://en.wikipedia.org/wiki/Inu_to_Hasami_wa_Tsukaiy%C5%8D)
