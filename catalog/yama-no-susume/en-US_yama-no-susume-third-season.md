# Yama no Susume: Third Season

-   **type**: tv-serie
-   **episodes**: 13
-   **original-name**: ヤマノススメ サードシーズン
-   **start-date**: 2018-07-03
-   **end-date**: 2018-07-03
-   **opening-song**: "Chiheisen Stride (地平線ストライド)" by Aoi (Yuka Iguchi), Hinata (Kana Asumi), Kaede (Yoko Hikasa), Kokona (Yui Ogura)
-   **ending-song**: "Irochigai no Tsubasa (色違いの翼)" by Aoi (Yuka Iguchi), Hinata (Kana Asumi)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   adventure
-   comedy
-   slice-of-life

## Sinopse

Aoi and Hinata are childhood friends. Aoi prefers staying indoors and is afraid of heights, whereas Hinata is outgoing and loves mountaineering. They both decide to climb a mountain in order to see a sunrise they saw together when they were younger.

In the coming third season, they enjoy hiking beautiful mountains with colored leaves and the story gets even more colorful when they become friends with local school girls!!

(Source: Yomiuri TV Enterprise)

## Links

-   [My Anime list](https://myanimelist.net/anime/35672/Yama_no_Susume__Third_Season)
-   [Official Site](http://www.yamanosusume.com/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=13205)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=20513)
-   [Wikipedia](http://en.wikipedia.org/wiki/Encouragement_of_Climb)
