# Kill Me Baby

-   **type**: tv-serie
-   **episodes**: 13
-   **original-name**: キルミーベイベー
-   **start-date**: 2012-01-06
-   **end-date**: 2012-01-06
-   **opening-song**: "Kill Me no Baby! (キルミーのベイベー!)" by Yasuna to Sonya (Chinatsu Akasaki & Mutsumi Tamura)
-   **ending-song**: "Futari no Kimochi no Honto no Himitsu (ふたりのきもちのほんとのひみつ)" by Yasuna to Sonya (Chinatsu Akasaki & Mutsumi Tamura)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   comedy
-   school
-   seinen

## Sinopse

Kill Me Baby is the touching story of Yasuna, a normal (?) high school girl, and Sonya, her best friend who happens to be an assassin. Unfortunately, little Sonya's trained assassin instincts often work against her and others in her daily high school life, as Yasuna's often-broken wrist can attest to. She just wanted a hug, but she ended up with a broken neck. Isn't it sad? No, it's hilarious.

Not even Yasuna's intense ninja training can prepare her for the exciting adventures in this explosive 4-panel manga adaptation.

## Links

-   [My Anime list](https://myanimelist.net/anime/11079/Kill_Me_Baby)
-   [Official Site](http://www.tbs.co.jp/anime/kmb/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=8552)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=13289)
-   [Wikipedia](http://en.wikipedia.org/wiki/Baby,_Please_Kill_Me!)
