# High School DxD New

![high-school-dx-d-new](https://cdn.myanimelist.net/images/anime/12/47729.jpg)

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: ハイスクール D×D NEW
-   **start-date**: 2013-07-07
-   **end-date**: 2013-07-07
-   **opening-song**: #1: "Sympathy" by Larval Stage Planning (eps 1-6)
-   **ending-song**: #1: "Houteishiki wa Kotaenai (方程式は答えない)" by Occult Kenkyuubu Girls (Yoko Hikasa, Shizuka Itou, Azumi Asakura, Ayana Taketatsu) (eps 1-6)
-   **rating**: R+ - Mild Nudity

## Tags

-   action
-   harem
-   comedy
-   demons
-   romance
-   ecchi
-   school

## Sinopse

The misadventures of Issei Hyoudou, high school pervert and aspiring Harem King, continue on in High School DxD New. As the members of the Occult Research Club carry out their regular activities, it becomes increasingly obvious that there is something wrong with their Knight, the usually composed and alert Yuuto Kiba. Soon, Issei learns of Kiba's dark, bloody past and its connection to the mysterious Holy Swords. Once the subject of a cruel experiment, Kiba now seeks revenge on all those who wronged him.

With the return of an old enemy, as well as the appearance of two new, Holy Sword-wielding beauties, it isn't long before Issei and his Devil comrades are plunged into a twisted plot once more.

[Source My Anime List]

## Links

-   [My Anime list](https://myanimelist.net/anime/15451/High_School_DxD_New)
-   [Official Site](http://haremking.tv/season2/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=9409)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=15204)
-   [Wikipedia](http://en.wikipedia.org/wiki/High_School_DxD)
