# Sayonara Zetsubou Sensei

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: さよなら絶望先生
-   **start-date**: 2007-07-08
-   **end-date**: 2007-07-08
-   **opening-song**: #1: "Hito toshite Jiku ga Bureteiru (人として軸がぶれている)" by Kenji Ohtsuki feat. Ai Nonaka, Marina Inoue, Yu Kobayashi, Miyuki Sawashiro & Ryoko Shintani (eps 1-9, 12)
-   **ending-song**: "Zessei Bijin (絶世美人)" by Ai Nonaka, Marina Inoue, Yu Kobayashi & Ryoko Shintani
-   **rating**: R - 17+ (violence & profanity)

## Tags

-   comedy
-   parody
-   school
-   shounen

## Sinopse

Nozomu Itoshiki is a high school teacher so pessimistic that even the smallest of misfortunes can send him into a pit of raging despair; some of these "catastrophes" even lead to suicide attempts. Sayonara Zetsubou Sensei is a satirical slice-of-life comedy set in the modern day, covering various aspects of Japanese life and culture through Nozomu and his interactions with his students: Kiri Komori, a recluse who refuses to leave the school; Abiru Kobushi, an enigma who frequently arrives to class with severe and mysterious injuries; the hyper-optimistic Kafuuka Fuura, Nozomu's polar opposite; and several other unusual girls, all of whom are just as eccentric as their teacher.

[Source My Anime List]

## Links

-   [My Anime list](https://myanimelist.net/anime/2605/Sayonara_Zetsubou_Sensei)
-   [Official Site](http://www.starchild.co.jp/special/zetsubou/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=5083)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=7767)
-   [Wikipedia](http://en.wikipedia.org/wiki/Sayonara,_Zetsubou-Sensei)
