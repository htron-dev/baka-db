# Blue Dragon: Tenkai no Shichi Ryuu

-   **type**: tv-serie
-   **episodes**: 51
-   **original-name**: BLUE DRAGON 天界の七竜
-   **start-date**: 2008-04-05
-   **end-date**: 2008-04-05
-   **opening-song**: #1: "Hikari no Sasu Hō e (光の指す方へ)" by JiLL-Decoy association (eps 1-26)
-   **ending-song**: #1: "Tsubomi (蕾)" by JiLL-Decoy association (eps 1-14)
-   **rating**: PG - Children

## Tags

-   adventure
-   comedy
-   fantasy
-   supernatural

## Sinopse

After the events of the Blue Dragon series, Shu and Bouquet join a resistance group to fight General Rogi's army of invasion.They encounter a mysterious child , Noi, who has the power to revive Shu's shadow counter-part Blue Dragon, and save her from the pursuit of a red dragon. Now Shu and his company have to set foot once again on a journey to discover the purpose of the dragons, undergo their tests and stop the chains of events which threatens the fate of humanity.

(Source: ANN)

## Links

-   [My Anime list](https://myanimelist.net/anime/4197/Blue_Dragon__Tenkai_no_Shichi_Ryuu)
-   [Official Site](http://www.tv-tokyo.co.jp/anime/bluedragon/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=5869)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=9815)
-   [Wikipedia](http://en.wikipedia.org/wiki/Blue_Dragon)
