# Eightman

![eightman](https://cdn.myanimelist.net/images/anime/4/73670.jpg)

-   **type**: tv-serie
-   **episodes**: 56
-   **original-name**: エイトマン
-   **start-date**: 1963-11-07
-   **end-date**: 1963-11-07
-   **opening-song**: "8 Man (エイトマン)" by Shigeru Katsumi (克美しげる)
-   **ending-song**: "Eight Man (instrumental version)" by Shigeru Katsumi
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   action
-   mecha
-   drama
-   sci-fi

## Sinopse

A private investigator working a routine case involving stolen technology is mortally wounded. His only hope for survival comes from sacrificing his humanity to become 8Man—a powerful cyborg crime-fighter enhanced with a living human brain. Resurrected as the ultimate high-tech vigilante, it is up to 8Man to bring the lawless to justice and put an end to the escalating cycle of violence. A classic animated thriller that will leave you breathless.

(Source: AnimeNfo)

## Links

-   [My Anime list](https://myanimelist.net/anime/5052/Eightman)
-   [Official Site](http://www.eiken-anime.jp/works/works003.html)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=1352)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=665)
-   [Wikipedia](http://en.wikipedia.org/wiki/8_Man)
