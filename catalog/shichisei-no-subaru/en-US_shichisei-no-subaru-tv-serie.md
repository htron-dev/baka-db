# Shichisei no Subaru

![shichisei-no-subaru](https://cdn.myanimelist.net/images/anime/1956/94914.jpg)

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: 七星のスバル
-   **start-date**: 2018-07-06
-   **end-date**: 2018-07-06
-   **opening-song**: "360° Hoshi no Orchestra (360° 星のオーケストラ)" by petit milady (eps 2-7, 9-12)
-   **ending-song**: #1: "360° Hoshi no Orchestra (360° 星のオーケストラ)" by petit milady (ep 1)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   action
-   fantasy
-   game
-   sci-fi

## Sinopse

Asahi's life in reality ended simultaneously when she died playing Union, a world-renowned online RPG. The members of Subaru went separate ways when Union shut down due to Asahi's death. Six years later, Haruto logs onto a renewed version of Re'Union and reunites with Asahi who had died. Is it really her or just an electronic ghost? The former team members reunite and enter the mysterious and deadly online game to find out.

(Source: TBS Global Business)

## Links

-   [My Anime list](https://myanimelist.net/anime/36316/Shichisei_no_Subaru)
-   [Official Site](http://7subaru.jp/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=13439)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=20850)
-   [Wikipedia](http://en.wikipedia.org/wiki/Seven_Senses_of_the_Re%27Union)
