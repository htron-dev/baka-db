# Coyote Ragtime Show

-   **type**: tv-serie
-   **episodes**: 12
-   **original-name**: コヨーテ ラグタイムショー
-   **start-date**: 2006-07-04
-   **end-date**: 2006-07-04
-   **opening-song**: "COYOTE" by Naoki with Power Sound
-   **ending-song**: #1: "Usuragu Kioku" by Sana (eps 1-11)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   action
-   adventure
-   comedy
-   mecha
-   sci-fi
-   shounen

## Sinopse

Mister is a "coyote" or space faring outlaw who has been sitting in prison for a year for a traffic offense. Ten days from release, he breaks out with the help of his old partners Bishop and Katana. He then seeks out Franka who has been left in his care by her dead father and takes her on a journey to find her father's treasure. On their heels are the federal investigators Angelica and Chelsea as well as the android assassins of the Criminal Guild, Madame Marciano's Twelve Sisters.

(Source: ANN)

## Links

-   [My Anime list](https://myanimelist.net/anime/1194/Coyote_Ragtime_Show)
-   [Official Site](http://www.ufotable.com/coyote/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=4327)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=6451)
-   [Wikipedia](http://en.wikipedia.org/wiki/Coyote_Ragtime_Show)
