# MÄR

![mar](https://cdn.myanimelist.net/images/anime/8/38383.jpg)

-   **type**: tv-serie
-   **episodes**: 102
-   **original-name**: メル
-   **start-date**: 2005-04-03
-   **end-date**: 2005-04-03
-   **opening-song**: #1: "Kimi no Omoi Egaita Yume Atsumeru HEAVEN" by Garnet Crow (eps 1-26)
-   **ending-song**: #1: "I Just Wanna Hold You Tight" by Miho Komatsu (eps 001-013)
-   **rating**: PG-13 - Teens 13 or older

## Tags

-   action
-   adventure
-   comedy
-   fantasy
-   shounen

## Sinopse

Dreaming of a magical world every night, the young Toramizu Ginta yearns to be able to go there. With only his friend Koyuki believing in his dreams, Ginta remains positive despite the slander he receives from others over his dreams. But his wishes are answered, as one day a large door appears in front of Ginta, summoning him to the land of MAR Heaven. In this land, the weapons known as ARMS exist. While initially Ginta greatly enjoyed the discovery of this magical world, he soon learns of the terrible wars that have once plagued MAR Heaven and the upcoming war that may soon appear.

## Links

-   [My Anime list](https://myanimelist.net/anime/738/MÄR)
-   [Official Site](http://marheaven.com/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=2855)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=4882)
-   [Wikipedia](http://en.wikipedia.org/wiki/M%C3%84R)
