# Kindaichi Shounen no Jikenbo Returns 2nd Season

-   **type**: tv-serie
-   **episodes**: 22
-   **original-name**: 金田一少年の事件簿R【リターンズ】
-   **start-date**: 2015-10-03
-   **end-date**: 2015-10-03
-   **opening-song**: #1: "Yonjuushi (四銃士)" by NEWS (eps 1-13)
-   **ending-song**: #1: "Hitomi no Oku no Milky Way (瞳の奥の銀河（ミルキーウェイ）)" by Flower (eps 1-12)
-   **rating**: R - 17+ (violence & profanity)

## Tags

-   mystery
-   shounen

## Sinopse

Hajime Kindaichi once again becomes embroiled in solving baffling cases and deciphering puzzling crimes that would confound the most seasoned of detectives. Whether it's a gruesome murder and shady circumstances surrounding the Japanese board game Go; a perplexing and macabre case involving a mysterious character, "Rosenkreutz," and blue roses; or blood curdling crimes associated with an urban legend at a winter ski resort – Hajime is out to crack them all!

(Source: YTV)

## Links

-   [My Anime list](https://myanimelist.net/anime/31227/Kindaichi_Shounen_no_Jikenbo_Returns_2nd_Season)
-   [Official Site](http://www.toei-anim.co.jp/tv/kindaichi_r/)
-   [AnimeDB](http://anidb.info/perl-bin/animedb.pl?show=anime&aid=11367)
-   [AnimeNewsNetwork](http://www.animenewsnetwork.com/encyclopedia/anime.php?id=17530)
-   [Wikipedia](http://en.wikipedia.org/wiki/Kindaichi_Case_Files)
